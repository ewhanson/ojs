<?php
/**
 * @file classes/issue/DAO.inc.php
 *
...
class DAO extends EntityDAO implements \PKP\plugins\PKPPubIdPluginDAO
{
    ...
    /**
     * Get issue IDs that have a matching setting
     */
    public function getIdsBySetting(string $settingName, $settingValue, int $contextId): Enumerable
    {
        return DB::table($this->table, 'i')
            ->join($this->settingsTable . ' as is', 'i.issue_id', '=', 'is.issue_id')
            ->where('is.setting_name', '=', $settingName)
            ->where('is.setting_value', '=', $settingValue)
     ...
     *
     */
    public function deleteCustomIssueOrdering(int $contextId, int $issueId = null)
    {
        $q = DB::table('custom_issue_orders')
            ->where('journal_id', '=', $contextId);

        if (!is_null($issueId)) {
            $q->where('issue_id', '=', (int) $issueId);
        }
     ...
     *
     */
    public function setDefaultCustomIssueOrders(int $contextId)
    {
        $publishedIssues = Repo::issue()->getPublishedIssues($contextId);
        $publishedIssues->each(function ($item, $key) {
        });
    }
    ...
    {
        $issues = Repo::issue()->getMany(Repo::issue()->getCollector()->filterByContext($contextId));
    ...
    private function _fromRow($row)
    {
        ...
        $issue->setDatePublished($this->deprecatedDao->datetimeFromDB($row['date_published']));
        $issue->setDateNotified($this->deprecatedDao->datetimeFromDB($row['date_notified']));
        $issue->setLastModified($this->deprecatedDao->datetimeFromDB($row['last_modified']));
        $issue->setAccessStatus($row['access_status']);
        $issue->setOpenAccessDate($this->deprecatedDao->datetimeFromDB($row['open_access_date']));
        ...
        $this->deprecatedDao->getDataObjectSettings('issue_settings', 'issue_id', $row['issue_id'], $issue);
